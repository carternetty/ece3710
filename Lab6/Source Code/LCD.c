...
void delay_ms(unsigned int ms) {					//Delays for a certain amount of time
	...
	while(dummyCount < ms * 50) {					//perform dummy task for certain time
		dummyCount++;
	}
	return;
}
...
void LCD_Init(void) {																		//Initialize
	...
	GPIOB->MODER |= 0x5555;
	GPIOB->OTYPER |= 0x00F0;
	//enable gpioc 0,1,2,3 as inputs
...
void LCD_DisplayStopwatch( volatile int time[]){
	LCD_WriteCom(0x80);
